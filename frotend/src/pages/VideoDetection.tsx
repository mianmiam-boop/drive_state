...
import { Video, Upload, Play, X, AlertTriangle, CheckCircle, Download } from 'lucide-react';
...
export default function VideoDetection() {
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      if (err.code === 'ECONNABORTED') {
        setError('检测超时，视频文件可能过大');
      } else if (err.response?.status === 401) {
        setError('未授权，请重新登录');
      } else if (err.response?.status === 404) {
        setError('检测接口不存在，请确认后端服务已启动，API地址：/api/detect/video');
      } else {
  ...
  const downloadResult = () => {
    ...
    link.href = url;
    link.download = `video_detection_${Date.now()}.json`;
    link.click();
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">视频监测</h2>
            <p className="text-sm text-gray-500 mt-0.5">上传视频文件进行疲劳检测分析</p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
                <h3 className="text-sm font-medium text-gray-700">上传视频</h3>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              ...
              {!result ? (
                <div className="p-12 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-2">上传视频并点击"开始检测"</p>
                </div>
              ) : (
